// https://raw.githubusercontent.com/dotnet/vscode-csharp/ba156937926e760759a7e5e13bcf2d1be8729dc8/src/lsptoolshost/dotnetRuntimeExtensionResolver.ts
...
export const DotNetRuntimeVersion = '10.0';
...
export class DotnetRuntimeExtensionResolver implements IHostExecutableResolver {
    ...
    private getEnvironmentVariables(dotnetExecutablePath: string): NodeJS.ProcessEnv {
        ...
        if (this.options.crashDumpPath) {
            ...
            env.DOTNET_DbgMiniDumpName = path.join(this.options.crashDumpPath, '%e.%p.dmp');
        }
    ...
    private async acquireRuntime(): Promise<HostExecutableInformation> {
        ...
        let status = await vscode.commands.executeCommand<IDotnetAcquireResult>('dotnet.acquireStatus', {
            ...
            requestingExtensionId: this.extensionId,
        });
        if (status === undefined) {
     ...
     * This is adapted from similar O# server logic and should be removed when we have a stable acquisition extension.
    ...
    private async findDotnetFromPath(): Promise<string | undefined> {
        try {
            ...
            // If the extension architecture is defined, we check that it matches the dotnet architecture.
